...
if 'X_input' in locals() and not X_input.empty:
    ...
    try:
        ...
        if X_input.shape[0] > 1:
            ...
            expected_value_single = shap_values.base_values[instance_to_explain_idx]
            shap_vals_class1_single = shap_values.values[instance_to_explain_idx]
    ...
    # Use shap.Explanation object with shap.plots.force()
    shap_plot = shap.plots.force(
        shap_values.base_values[instance_index],
        shap_values.values[instance_index],
        matplotlib=True  # Note: this works visually but is not JS-embeddable in Streamlit
    )
    st.pyplot(shap_plot)
    # Streamlit rendering
